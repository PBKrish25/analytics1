...
#use apply functions
sapply(inputData, sd)#simple apply format
lapply(inputData, sd)#list apply format
...
(t1= xtabs(~ admit + rank, data = data))#xtabs is changing the format
addmargins(t1)#calculating sum row-wise and column-wise
prop.table(t1,2)#calculating proportions or percentages of the rows
?prop.table
#xtabs(~ gear + cyl + am , data=mtcars)
#create Logistic Model
mylogit = glm(admit ~ gre + gpa + rank, data = data, family = "binomial")

summary(mylogit)#understand the concept of dummy variables
#questions to be answered
#which are the variables which are significant?
...
(ndata2 = data.frame(gre=c(600, 700), gpa=c(2,3), rank=factor(c(1,2))))
(p2= predict(mylogit,newdata=ndata2, type=c("response")))
#predicting that if values<0.5, he wont be selected and vice-versa
